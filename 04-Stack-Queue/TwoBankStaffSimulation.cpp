//
// Created by sinil on 2025-10-18.
//
...
int remove_customer()
{
	...
	return service_time; // customer.service_time - 1을 반환하게된다.
}

int remove_customer(int teller_id)
{
	element customer;
	int service_time = 0;
	...
	served_customers++;
	waited_time += clock - customer.arrival_time;
	printf("[STAFF %d] Customer %d starts service at %d min (Wait %d min)\n",
		  teller_id, customer.id, clock, clock - customer.arrival_time);
	return service_time; // customer.service_time - 1을 반환하게된다.
}
...
int main()
{
	...
	while (clock < duration) {
		...
		if (service_time1 == 0) {
			service_time1 = remove_customer(1); // 여기에서 서비스타임을 새로운 손님마다 초기화하는거!
		}
		if(service_time2 == 0) {
			service_time2 = remove_customer(2); // 두개의 서비스타임으로 동시에 굴린다.
		}
		//어떤 은행 스태프가 처리하고있는 지 보기위해, remove_customer 를 오버로딩했다. 앞에 처리하고있는 스태프의 번호가 출력되도록 한줄 추가함.
	}
	print_stat();
	return 0;
}
